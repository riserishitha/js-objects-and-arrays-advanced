...
const s = strings.filter((str) => str.length >= 5);
console.log(s);
...
const alphabets = ['K', 'a', 'l', 'v', 'i', 'u', 'm'].reduce(function (
  result,
  item
) {
  return result + item;
...
const first = number.find((num) => num % 2 === 0);
console.log(first);
...
const sec = ranks.findIndex((num) => num === 7);
console.log(sec);
...
const audiString = `1."${cars[0].name}"`;
console.log(audiString);
const toyota = cars.find((car) => car.name === 'Toyota');
const o = `2. "${toyota.name}  ${toyota.color[0]}"`;
console.log(o);
const hyundai = cars.find((car) => car.name === 'Hyundai');
const t = `3. "${hyundai.name} ${hyundai.color[2]} ${hyundai.capacity}"`;
console.log(t);
const renault = cars.find((car) => car.name === 'Renault');
const b = `4. "${renault.name} ${renault.color[1]} ${renault.capacity}"`;
console.log(b);
...
const u = `"Mr ${person['name']} travels by plane called Air123".`;
console.log(u);
